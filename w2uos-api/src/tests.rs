...
async fn control_endpoints_drive_state_and_audit_log() {
    ...
    let live_switch_req = test::TestRequest::post()
        .uri("/control/live_switch")
        .insert_header((HeaderName::from_static("x-api-key"), "trader-key"))
        .set_json(&json!({"armed": true, "reason": "test_arm"}))
        .to_request();
    let live_switch_resp = test::call_service(&app, live_switch_req).await;
    assert!(live_switch_resp.status().is_success());

    let control_state_req = test::TestRequest::get()
        .uri("/control/state")
        .insert_header((HeaderName::from_static("x-api-key"), "trader-key"))
        .to_request();
    let control_state_resp = test::call_service(&app, control_state_req).await;
    assert!(control_state_resp.status().is_success());
    let state_body = to_bytes(control_state_resp.into_body()).await.unwrap();
    let state_decoded: crate::types::ControlStateDto = serde_json::from_slice(&state_body).unwrap();
    assert!(state_decoded.live_switch_armed);
