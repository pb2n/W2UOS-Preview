...
pub struct ExchangeConfig {
    /// Identifier for the configured exchange (e.g., "okx" or "binance").
    #[serde(rename = "name", deserialize_with = "deserialize_exchange_kind")]
    pub exchange: ExchangeId,
    /// Execution tier for this exchange adapter (simulation, paper, or live).
    #[serde(
        rename = "mode",
        default = "ExecutionMode::default",
        deserialize_with = "deserialize_exchange_execution_mode"
    )]
...
fn deserialize_exchange_execution_mode<'de, D>(deserializer: D) -> Result<ExecutionMode, D::Error>
where
    D: serde::Deserializer<'de>,
{
    ...
    Ok(match normalized.as_str() {
        "simulation" | "sim" => ExecutionMode::Sim,
        "live-paper" | "paper" => ExecutionMode::Paper,
        "live-real" | "live" => ExecutionMode::Live,
        other => {
...
fn deserialize_exchange_kind<'de, D>(deserializer: D) -> Result<ExchangeId, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let value = String::deserialize(deserializer)?;
    Ok(ExchangeId::from(value.as_str()))
}
