...
impl ExecutionService {
    ...
    async fn handle_command(&self, cmd: OrderCommand) -> Result<OrderResult> {
        ...
        if matches!(mode, ExecutionMode::Live) && !Self::live_checks_pass(&cfg) {
            return Ok(OrderResult {
                command_id: cmd.id,
                exchange: cmd.exchange,
                symbol: cmd.symbol,
                ...
                status: OrderStatus::Rejected(
                    "live execution requires config.live_trading_enabled=true and W2UOS_CONFIRM_LIVE=YES_I_UNDERSTAND"
                        .to_string(),
                ),
                filled_size_quote: 0.0,
                avg_price: 0.0,
