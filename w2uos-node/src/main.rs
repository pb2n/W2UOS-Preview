...
async fn main() -> Result<()> {
    ...
    let exchange_data_cfg: Option<ExchangeDataConfig> =
        exchange_cfg.as_ref().map(|ex| ExchangeDataConfig {
            ...
            mode: match ex.execution_mode {
                ExecutionMode::Sim => ExchangeDataMode::Sim,
                ExecutionMode::Paper => ExchangeDataMode::Paper,
                ExecutionMode::Live => ExchangeDataMode::Live,
            },
    ...
    let configured_execution_mode: ExecutionMode = exchange_cfg
        .as_ref()
        .map(|ex| ex.execution_mode.clone())
        .unwrap_or(ExecutionMode::Sim);

    exec_config.execution_mode = configured_execution_mode.clone();

    exec_config.paper_trading = matches!(configured_execution_mode, ExecutionMode::Paper);
    ...
    let exchange_profile = exchange_cfg
        .as_ref()
        .map(|ex| (ex.exchange.to_string(), format!("{:?}", ex.execution_mode)));
